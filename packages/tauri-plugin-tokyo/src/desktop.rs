...
impl<R: Runtime> Library<R> {
  ...
  pub async fn get_index(&self, name: String) -> crate::Result<Vec<tokyo_files::IndexEntry>> {
    let dir = tokyo_files::Library::find_library(&self.db, name.as_str())
      .await
      .unwrap()
      .path;
    let index = tokyo_files::Library::get_index(&self.db, dir).await;
    return Ok(index);
  }

  pub async fn get_metadata(&self, file_path: String) -> crate::Result<tokyo_files::MetadataEntry> {
    let meta = tokyo_files::Library::metadata(&self.db, &file_path).await;
    if let Some(metadata) = meta {
      return Ok(metadata);
    }
