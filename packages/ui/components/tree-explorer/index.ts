    ...
    render() {
        ...
        const renderItem = item => {
            ...
            const dropHandler = e => {
                ...
                const childIndex = e.dataTransfer.getData('item').split(',').map(n => parseInt(n));
                let order = 0;
            ...
            return html`
                <div class="layer" ?even="${oddEven}" ?odd="${!oddEven}" ?collapsed=${!item.uncollapsed}>
                    <div class="item"
                        ...
                        ${(item.children && item.children.length) > 0 ? html`<span class="collapse-btn" @mousedown=${e => this.toggleCollapse(item)}></span>` : ''}
